...
def generate_data(df : pd.DataFrame) -> None:
    for i in range(0, 100):
        ...
        data = [agent_id, pp, P_1, P_2, P_3, a_enjoy_camp, a_enjoy_resort, a_budget_camp, a_budget_resort, a_conform_chain, a_conform_independent, a_stim_chain, a_stim_independent, a_enjoy_classic, a_enjoy_unknown, a_stimulation_classic, a_stimulation_unknown]
        df.loc[len(df)] = data

if __name__ == '__main__':
    df = pd.DataFrame(columns=['agent_id', 'pp', 'P_1', 'P_2', 'P_3', 'a_enjoy_camp', 'a_enjoy_resort', 'a_budget_camp', 'a_budget_resort', 'a_conform_chain', 'a_conform_independent', 'a_stim_chain', 'a_stim_independent', 'a_enjoy_classic', 'a_enjoy_unknown', 'a_stimulation_classic', 'a_stimulation_unknown'])
    generate_data(df)
