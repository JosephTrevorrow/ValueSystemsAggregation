...
def output_file(p,U,cons,dist_1,dist_l,v,name):
    ...
    """
    csv_rows = []
    if v: #header for the value prefference aggregation
        header = ["p","Up","Dist1","Distl","Rel-Rel","Rel-Nonrel","Nonrel-Rel","Nonrel-Nonrel"]
    else: #header for the aggregation of moral values
    ...
    for i in range(len(p)):
        ...
        for j in range(len(cons[i])): #writing consensus
            el.append(cons[i][j])
        csv_rows.append(el)
        
    with open(name, 'w', newline='') as csvfile: #writing file
